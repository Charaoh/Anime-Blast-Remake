...
if (!$account) {
	...
	if ($system->data('activation') === '1' && $account['activated'] !== '1' && strpos($current_url, '?s=activate') === false) {
		...
		$activate = '<a href="' . $siteaddress . 'activation"><p style="
    margin-top: 10px;
    color: #df193c;
    font-size: 10px;
...
"><span style="
    ...
    float: left;
    text-align: center;
">Activate your account for an exclusive character!</span><img src="https://www.anime-blast.com/images/characters/91.jpg" style="
    width: 30px;
    border: 1px solid black;
    margin-right: 2px;
"><img src="https://www.anime-blast.com/images/characters/93.jpg" style="
    width: 30px;
    border: 1px solid black;
    margin-right: 2px;
"><img src="https://www.anime-blast.com/images/characters/94.jpg" style="
    width: 30px;
    border: 1px solid black;
"></p> </a>';
	}
...
$output = $STYLE->tags($output, array("LANDSCAPE" => ($landscape) ? '<link rel="manifest" href="./manifest.json">' : '', "SCRIPTS" => $scripts, "METAINFO" => $description, "METAKEYWORDS" => $metatags, "ACTIVATE" => $activate, "RANKED" => $ladders, "RANKED-CLAN" => $clanrank, "RANKED-STREAK" => $streaks, "URL" => $url, "GLOBAL_MENU" => $global_menu, "AREA" => $page_title, "SITELINK" => '<a href="' . $siteaddress . '" class="normfont">' . $sitename . '</a>', "PAGETITLE" => strip_tags($page_title), "VERSION" => $version));
