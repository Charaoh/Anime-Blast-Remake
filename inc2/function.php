...
class system
{
    function sendDiscordMsg($msg = "Hello World!", $url = 1)
    {
        if ($url === 1)
    ...
    function manageOnline($check = false)
    {
        global $db, $id, $session_id, $ip, $session_location;
        if ($check) {
            return $db->query("SELECT * FROM online WHERE account_id = '$check'")->rowCount() > 0;
        }
    ...
    function bbcode($string)
    {
        ...
        $string = str_replace(array('[quote]', '[/quote]'), array($open, $close), $string);
    ...
    function keywordReplacements($string)
    {
        ...
        if ($number) {
            ...
            while ($number_two > 0) {
                ...
                if ($count > $low && $count < $high) {
                    $value .= '<a href="' . $relay . '&amp;page=' . $count . '" class="pagefont">' . $count . ' </a><p class="pagefont"> | </p>';
                }
            }
...
class secure
{
    ...
    {
        return addslashes(strip_tags($content));
    }
...
">'.$avatar.'</div>;*/
    ...
    function image($id, $dir = 'characters', $path = './', $class = 'default', $sub_id = '', $add = '', $default = true)
    {
    ...
    function status($user_id, $section = null)
    {
        global $db;
    ...
    function getCharacter($pCharacterID)
    {
...
class game
{
    ...
    function getBrowser()
    {
        ...
        if (isset($_SERVER["HTTP_USER_AGENT"])) {
            ...
            if (false !== stripos($u_agent, "MSIE") && !preg_match('/Opera/i', $u_agent)) {
