...
if ($mode == 'edit' && isset($_GET['post_id'])) {
    ...
    if (empty($post_style)) {
        $post_style = '<td class="whiter">No comments found</td>';
    }
    ...
    if (strpos($topic_id, 'c') !== false) {
        ...
        while ($searching = $search->fetch()) {
            ...
            foreach ($unlocks as $unlock) {
                ...
                if ($unlock == $topic_data['id']) {
                    ...
                    $requirement .= 'Completing the mission <a href="' . $siteaddress . 'missions#mission' . $searching['id'] . '" style="margin:auto .5em; color:#9c1120;">"' . $searching['name'] . '"</a>';
                }
            }
        }
        /* $character .= '<div class="container">
        <div class="character-layout" style="width: 75%;margin: 0 auto;">
        <h2>' . $user->image($topic_data['who'], 'animes', './', 'filter') . $topic_data['name'] . '</h2>
        <p>' . $user->image($topic_data['id'], 'characters/slanted', './', 'default') . ' ' . $topic_data['desc'] . '<br class="clearfix">
        <div class="information"><p class="viewc">Requirements to unlock: ' . $requirement . '</p>
        <p class="hp_mana">HP <span style="background: #51d964;">' . $topic_data['health'] . '</span></p>
        <br class="clearfix">
        <p class="hp_mana">MANA <span style="background: #34d0f1;">' . $topic_data['mana'] . '</span></p>
        </div></div>';
         */

        $character .= '
            <div class="flex mx-auto p-2 character-profile">
            <div class="coverlay">
            <h2>' . $user->image($topic_data['who'], 'animes', './', 'filter') . $topic_data['name'] . '</h2>
        ...
        if (!empty($topic_data['passive'])) {
            ...
            foreach ($passives as $passive) {
                ...
                if ($passive) {
                    ...
                    <h2 style="border-bottom-width: 1px;font-size: 12px;"><span style="color: white;">Passive: </span>
					' . $passive['name'] . '</h2>
            ...
            if (!empty($add)) {
				...
				' . $add . '
				<p style="color: #d71613;text-align: center;font-size: 11px;">*Passives trigger automatically when the match starts*</p>
                <br class="clearfix">
                </div></div></div></div>';
            }
        }
        ...
        $skilleffects = explode(',', $topic_data['skilleffects']);
        $passives = explode(',', $topic_data['passive']);
        $skills = array_merge($skillset, $passives, $skilleffects);
        $alts = '';
        $keys = array();
        ...
        foreach ($skills as $_ => $skill) {
                    ...
                    do {
                        ...
                        foreach ($ez as $looks) {
                            ...
                            if ($alt->rowCount() > 0) {
                                ...
                                $alts .= '<div class="skill-layout alt" style="display:none;">
									<h2>' . $alt['name'] . '</h2>
									...
									<p>' . $system->keywordReplacements($alt['desc']) . '<br class="clearfix">
										...
										<div class="information">
											...
											<span class="hp-mana">' . $alt['cost'] . ' Mana</span>
											<br class="clearfix">
											' . $archive . '
										</div>
									</div>';
            ...
            if (array_search($skill, $passives) === false) {
				...
				<span class="hp-mana">' . $s['cost'] . ' Mana</span>
				<br class="clearfix">
				' . $archive . '
				</div>
                </div><!-- END skill_' . $_ . ' -->';
        ...
        $character .= '</div></div>';
        if (!empty($alts)) {
            $character .= '<h4 class="my-2 alternative" style="color:#ffffff; font-weight:bolder;">Alternative Skills</h1></div>' . $alts;
        }
    }
    ...
    if (isset($_POST['Submit'])) {
        ...
        if (isset($topic_data['locked']) && $topic_data['locked'] == '1') {
            goto cantpost;
        }
