...
if ($mode == 'view') {
    if (!isset($_GET['m'])) {
        $system->message(L_ERROR, L_MAIL_ERROR_ID, './mail', L_CONTINUE);
    }
    ...
    // Update Page Title
    $page_title .= ' > <a href="./mail" class="normfont">Inbox</a>';
    if (!$mail) {
        $system->redirect($siteaddress.'mail');
    }
    ...
    if (isset($_POST['Submit']) && isset($_POST['message'])) {
        ...
        $system->message(L_MAIL, L_MAIL_REPLY, './mail', L_CONTINUE);
    }
    $output .= $STYLE->tags($tpl, array("AVATAR" => $user->avatar($author_id),"BBCODES"=>$system->returnBBcodes(),
                "AUTHOR" => $user->name($author_id),
                "TEXT" => $system->bbcode($mail['text']),
                "RANK" => $user->rank($author_id),
                "STATUS" => $user->status($author_id, 'mail'),
    ...
    $page_title .= ' > <a href="./mail?mode=new" class="normfont">' . L_NEW_MAIL . '</a>';
    ...
    if (isset($_POST['Submit'])) {
        ...
        if (!isset($_POST['title']) || !isset($_POST['message']) || !isset($_POST['user'])) {
            $system->message(L_ERROR, L_INFORMATION_MISSING, './mail', L_CONTINUE);
        }
        // Nothing is Blank
        ...
        if (!$user_account) {
            $system->message(L_ERROR, L_USER_NOT_FOUND, './mail', L_CONTINUE);
        }

        $system->mail($user_account['id'], $account['id'], $secure->clean($_POST['title']), $secure->clean($_POST['message']));
        $system->message(L_SENT, L_MAIL_SENT, './mail', L_CONTINUE);
    }
    if (isset($_GET['user'])) {
    ...
    $output .= $STYLE->tags($tpl, array("BBCODES"=>$system->returnBBcodes(),"L_SUBMIT" => L_SUBMIT, "L_USER" => L_USER, "L_NEW_MAIL" => L_NEW_MAIL, "L_TITLE" => L_TITLE, "USERNAME" => $username
            ));
} else {
    ...
    if (isset($_POST['delete']) && isset($_POST['checkbox'])) {
        ...
        $system->message(L_DELETED, L_MAIL_SELECTED_DELETED, './mail', L_CONTINUE);
    }
    if ($mode == 'sent') {
        $page_title .= ' > <a href="./mail?mode=sent" class="normfont">' . L_SENTBOX . '</a>';
        ...
        $page_title .= ' > <a href="./mail" class="normfont">' . L_INBOX . '</a>';
    ...
    while ($mail_row = $mail_sql->fetch()) {
        ...
        $mail_style .= $STYLE->tags($mail_tpl, array("ID" => $mail_row['id'], "ICON" => $icon, "CLASS" => $class, "TITLE" => '<a href="./mail?mode=view&amp;m=' . $mail_row['id'] . '" class="normfont">' . $system->present($mail_row['title']) . '</a>', "DATE" => $system->time($mail_row['date']), "AUTHOR" => $author));
