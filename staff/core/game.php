<?php
$tpl = $STYLE->open('staff/ingame.tpl');
...
if ($mode == 'character') {
    if (isset($_GET['id'])) {
        ...
        if ($character->rowCount() > 0) {
            ...
            $tpl = $STYLE->open('staff/staffpanel.tpl');
    ...
    if (isset($_GET['id'])) {
        ...
        if ($skill->rowCount() > 0) {
            ...
            if (isset($_POST['delete'])) {
                ...
                $system->redirect('./?s=game&module=skill');
            }
            if (isset($_POST['add-effect'])) {
                ...
                $system->redirect('./?s=game&module=skill&id=' . $skill['id']);
            }
            if (isset($_POST['submit'])) {
            ...
            if (isset($_POST['Avi'])) {
                ...
                if ($image) {
                    ...
                    // Make sure it is an image
                    if ((($extension != "jpg") && ($extension != "jpeg") && ($extension != "png") && ($extension != "gif"))) {
                        $system->message(L_ERROR, 'The format uploaded is invalid', './?s=game&module=skill&id=' . $skill['id'] . '&change-avatar=true', L_CONTINUE);
                    }
                    // Delete possible existing avatar
                ...
                if (!isset($copied)) {
                    $system->message(L_ERROR, 'An error occured on upload, please try again.', './?s=game&module=skill&id=' . $skill['id'] . '&change-avatar=true', L_CONTINUE);
                } else
                if ($height > $system->data('avatar_height') || $width > $system->data('avatar_width')) {
                    // Prevent Avatar over Dimension size
                    unlink("$newname");
                    $error_message = str_replace(array("[HEIGHT]", "[WIDTH]"), array($system->data('avatar_height'), $system->data('avatar_width')), L_AVATAR_UPLOAD_DIMENSION);
                    $system->message(L_ERROR, $error_message, './?s=game&module=skill&id=' . $skill['id'] . '&change-avatar=true', L_CONTINUE);
                } else
                if ($size > $system->data('avatar_filesize')) {
                    // Prevent Avatar over File size
                    unlink("$newname");
                    ...
                    $system->message(L_UPDATED, 'Skill picture update', './?s=game&module=skill&id=' . $skill['id'], L_CONTINUE);
                }
            }
            $tpl = $STYLE->open('staff/skills.tpl');
            if (isset($_GET['change-picture'])) {
                $tpl = $STYLE->getcode('change-avatar', $tpl);
                $tpl = $STYLE->tags(
                    $tpl,
        ...
        if (isset($_POST['submit'])) {
            ...
            if (isset($_FILES)) {
                ...
                if ($image) {
                    ...
                    // Make sure it is an image
                    if ((($extension != "jpg") && ($extension != "jpeg") && ($extension != "png") && ($extension != "gif"))) {
                        $system->message(L_ERROR, 'The format uploaded is invalid', './?s=game&module=skill&new=true', L_CONTINUE);
                    }
                    // Delete possible existing avatar
                ...
                if (!isset($copied)) {
                    $system->message(L_ERROR, 'An error occured on upload, please try again.', './?s=game&module=skill&new=true', L_CONTINUE);
                } else
                if ($height > $system->data('avatar_height') || $width > $system->data('avatar_width')) {
                    // Prevent Avatar over Dimension size
                    unlink("$newname");
                    $error_message = str_replace(array("[HEIGHT]", "[WIDTH]"), array($system->data('avatar_height'), $system->data('avatar_width')), L_AVATAR_UPLOAD_DIMENSION);
                    $system->message(L_ERROR, $error_message, './?s=game&module=skill&new=true', L_CONTINUE);
                } else
                if ($size > $system->data('avatar_filesize')) {
                    // Prevent Avatar over File size
                    unlink("$newname");
                ...
                $system->message(L_ERROR, 'Please supply a skill picture.', './?s=game&module=skill&new=true', L_CONTINUE);
            }
            ...
            if ($last == true) {
                ...
                $system->message($_POST['name'], 'Was inserted to the database!', './?s=game&module=skill&id=' . $db->link_id->lastInsertId(), L_CONTINUE);
            } else {
                $system->message('An error occured', 'Insertion to the database was not possible.', './?s=game&module=skill&new=true', L_CONTINUE);
            }
        } else
        ...
        if ($skills->rowCount() > 0) {
            ...
            while ($skill = $skills->fetch()) {
                ...
                <td class="skilldiv-name" colspan="2" align="left"><a href="./?s=game&module=skill&id=' . $skill['id'] . '" class="skilldiv-name">' . $skill['name'] . '</a><br>
