 ...
 */
    ...
    function Waiting($_ = 0) {
		...
		if($template !== 'default'){
            ...
            <p>'.$db->fieldFetch('accounts', $this->first['ACCOUNT']['id'],'extraTitle').'</p>';
            ...
            <p>'.$db->fieldFetch('accounts', $this->second['ACCOUNT']['id'],'extraTitle').'</p>';
		}
		$mybg = '';
		if(!empty($account['bg'])){
								...
								foreach(reset($t['skill']) as $effect => $turnsleft){
									...
									if(!empty($e['counter'])){
										...
										if($e['counter'] !== 'Nothing'){
                                        	...
                                        	$me = ($truePlayerTurn == '' ? '0' : '1');
                                        	$castedBy = ($me == '0' ? '1' : '0'); 
											$return = $this->match['TURN'].'='. $castedBy . $__ .':' . $e['counter'] . '[' . $w . ']'. $me . $key ;
										}
							...
							foreach($checkme as $k => $v){
								...
								foreach(reset($t['skill']) as $effect => $turnsleft){
									...
									if(!empty($e['counter'])){
                                    	...
                                    	$me = ($castedby == '1' ? '0' : '1');
										$return =$this->match['TURN'].'='.$castedby.':' . $e['counter'] . '[' . $w . ']'.$me . $key ;
