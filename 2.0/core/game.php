...
if ($mode == 'selection') {
	...
	"LR" => $ladderrank,
	"GROUP" => $db->fieldFetch('usergroups',$account['group'],'title'),
	"FILTERS" => $filter,
	"ANIMES" => $animes,
	"CLASSES" => $cls,
	"EFFECTS" => $effects,
	...
	<source src="./tpl/default/sound/selection.mp3" />
		...
		$output .= '<script>let mvol="'.(isset($account['mvol'])&&$account['mvol'] !== ""?$account['mvol']:$system->data('default-volume-music')).'", vsfx="'.(isset($account['vsfx'])&&$account['vsfx'] !== ""?$account['vsfx']:$system->data('default-volume-sfx')).'", sfx = '.json_encode($out).', SFXpackage = "'.$account['sfx'].'"; Object.freeze(SFXpackage); Object.freeze(sfx); </script>';
	}
} elseif ($mode == 'search') {
	$account = $db->fetch("SELECT * FROM accounts WHERE id = '".$account['id']."'");
	$type = '';
