...
class Ajax {
	...
	function resultMatch(){
		...
		global $db, $account, $STYLE, $system, $json, $user;
		...
		if($rows->rowCount() > 0){
			...
			if($rows['resulted'] == 0 && $rows['check-0'] == '0' && $rows['check-1'] == '0'){
				...
				foreach($players as $key => $player){
					...
					if($rows['type'] == 'ladder'||$rows['type'] == 'quick'){
						switch($result){
							case 'winner':
								...
								if($rows['type'] == 'ladder'){
                                    ...
                                    // bellow is BC system formula
                                     if($rankEnemy>=10)
                                     $gold = (($system->data('Gold_Earn')*$extra+($levelDifference*rand(4,10))+rand(45+$rankMe,45+($rankEnemy)*5)))*2;
                                     else $gold = (($system->data('Gold_Earn')*$extra+($levelDifference*rand(4,10))+rand(45+$rankMe,95)))*2;
                                    // BC event just by adding an extra () and *2 to both cases of the if
				...
				$db->query("UPDATE `matches` SET `resulted` = '1', `reward` = '".$r."'  WHERE id = '".$rows['id']."'");
			}else{
