 ...
 */
    ...
    function Waiting($_ = 0) {
		...
		$player = $user->image($this->first['ACCOUNT']['id'], 'avatars', './', 'fl-r avatar').'
			...
			<p>'.$system->group($this->first['ACCOUNT']['group']).'</p>';
		$opponent = $user->image($this->second['ACCOUNT']['id'], 'avatars', './', 'fl-l avatar').'
			...
			<p>'.$system->group($this->second['ACCOUNT']['group']).'</p>';
		if($template !== 'default'){
    ...
    // AI MATCH ->
    	...
    	if($this->match['MATCH']['type'] === 'ai' && !$this->match['ME']){
                ...
                foreach ($this->first['TEAM']['ID'] as $key => $character) {
                    if($this->first['TEAM']['HEALTH'] == '0') continue;
                	$numSkills = 1;
                    ...
                    do{
                        $randIndex = array_rand($this->first['TEAM']['SKILL'][$key]);
					...
					continue;
                     ...
                     }while(!$selectedMove && $moveCounter < count($this->first['TEAM']['SKILLS'][$key]));
                        ...
                        foreach($moveTargets as $target){
								...
								if(strpos($target, 'E') !== false){
                                	if(strpos($target, 'a') !== false){
                                    	if ($this->checkInvulnerability($this->second['TEAM']['STATUS'][0]['invulnerability'], $move['classes']) === false && $move['iinvul'] == '0'){
                                    		if(isset($_POST['10-targeted']))
                                    			$_POST['10-targeted'] .= ',';
                                			$_POST['10-targeted'] .= $selectedMove;
                                        }
                                    	if ($this->checkInvulnerability($this->second['TEAM']['STATUS'][1]['invulnerability'], $move['classes']) === false && $move['iinvul'] == '0'){
                                    		if(isset($_POST['11-targeted']))
                                    			$_POST['11-targeted'] .= ',';
                                			$_POST['11-targeted'] .= $selectedMove;
                                        }
                                    	if ($this->checkInvulnerability($this->second['TEAM']['STATUS'][2]['invulnerability'], $move['classes']) === false && $move['iinvul'] == '0'){
                                    	...
                                    	$_target = explode('/',$target);
                                    	$_target = $_target[1];
                                    	if ($this->checkInvulnerability($this->second['TEAM']['STATUS'][$_target]['invulnerability'], $move['classes']) === false && $move['iinvul'] === '0'){
                    ...
                    foreach ($targeted as $w =>  &$me) {
						...
						if($random === true){
							...
							do {
								...
								if($this->{$identifier}['TEAM']['HEALTH'][$__] !== '0')
									if($this->checkInvulnerability($this->{$identifier}['TEAM']['STATUS'][$__]['invulnerability'], $skill) === false)
										$blank = true;
								} while ($blank === false);
							$l = $skill['id'];
						}
                        ...
                        if (!empty($_add)){
							...
							if($skill['unreflectable'] == '1')
								$ignore = true;
							if($this->{$identifier}['TEAM']['STATUS'][$__]['reflect'][1] !== 'all'){
								$classes = explode(',', $skill['classes']);
							...
							if(isset($this->{$identifier}['TEAM']['STATUS'][$__]['reflect']) && $this->{$identifier}['ACCOUNT']['id'] != $this->first['ACCOUNT']['id'] && $ignore === false){
								...
								foreach($checkme as $k => $v){
									$t = $this->makemeunderstand($v);
									if($this->{$identifier}['TEAM']['STATUS'][$__]['reflect'][0] != key($t['skill']))
										continue;
									foreach(reset($t['skill']) as $effect => $turnsleft){
										$e = $db->fetch("SELECT * FROM effects WHERE id = '".$effect."'");
										if($e['count'] !== '1') 
								...
								$active = $checkme;
                                $target = ($truePlayerTurn == '0' ? '1' : '0');
								$active[] = $this->match['TURN'].'='. $truePlayerTurn . $__. ':' . $skill['id'] . '[' . $_add . ']'. $target . $key;
