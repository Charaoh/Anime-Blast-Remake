let Listener, verify, scripts = document.getElementsByTagName("script"),
	src = scripts[scripts.length - 1].src, canSendAjax = true,
	_path = `./${gVersion}/`, cload = false, Timer = null, skipuiload = false,
...
for (key in efx) {
	efx[key] = new Howl({
		src: efx[key][0], html5: true, preload: true,
		volume: Number(vsfx)
	})
};
for (key in bg) {
	bg[key] = new Howl({
		src: bg[key][0], html5: true, preload: true,
		volume: Number(mvol)
	})
};
...
(function ($) {
	$.fn.updateAvatars = function (team) {
		...
		$.each(team, function (index, chunk) {
			...
			let tempc = $(team[index]['image']), cimage = $(d[index]).find('.character-frame').children(":first-child");
			...
			if (tempc.prop('outerHTML') !== cimage.prop('outerHTML')) {
				...
				$(d[index]).find('.damaged').show(0, function () { $(this).addClass('expand'), cimage.replaceWith(team[index]['image']); $(this).children(":first-child").replaceWith(team[index]['image']) });
				transform = true;
		...
		$.fn.updateBars = function (what, last, now) {
			let d = {
				"00": "slot.A",
				"01": "slot.B",
				"02": "slot.C",
...
! function (d) {
			...
			updateUI = function () {
				...
				d.ajax({
					...
					success: function (t) {
						...
						if (!skipuiload) {
							skipuiload = true;
							d.fn.updateBars('Healths', health, t.healths);
							health = t.healths;
							d.fn.updateBars('Manas', mana, t.manas);
							mana = t.manas;
			...
			return {
				...
				checkTarget: function (t) {
					...
					let i = {
						...
						"eslot B": "11",
						"eslot C": "12"
					}, e = s.closest(".slot").attr("class").split(" ")[1], a = available[i[e]][u]['available'].split("|"), tempcopy = { ...available[i[e]][u]['targets'] };
					let specific = false;
					for (var k = 0; k < a.length; k++) {
						istarget = false;
						if (i[r] >= 10) {
							// Enemy Target
							if (a[k] == 'E/1' || a[k] == 'E/r' || a[k] == 'E/a')
								istarget = true;
							if (a[k] == 'E/1')
								specific = true;
						} else {
							// Ally Target
							if (a[k] == 'A/1' || a[k] == 'A/a' || a[k] == 'A/r')
								istarget = true;
							if (a[k] == 'A/1' || a[k] == 'S')
								specific = true;
							if (a.indexOf('A/a') !== -1)
	...
	window.resizeTo(t + 16, e + 88), 1 == turn && efx["start"].play(), "loser" !== match_status && "winner" !== match_status && ($(window).startTimer(startTime, maxTime, turn), $(window).listener(match_status)), "loser" != match_status && "winner" != match_status || (me = !1, clearTimeout(Listener), Listener = null, "loser" == match_status ? bg["lose"].play() : bg["win"].play(), $.ajax({
		url: _path + "/core/ajax.php",
		type: "POST",
		data: {
			f: "return_",
	...
	$(".turnText,.endMatch").live("click", function (t) {
		...
		if ($(t.target).is('.transparency'))
			$('.transparency').fadeOut();
	}); let character; $(".character-frame, .hover").live("click", function (t) {
		...
		let sid = $(this).attr('id'); sid = Object.keys(character["SKILLS"]).find(k => character["SKILLS"][k]["ID"] === sid);
	...
	$(".plus").on("click", function (e) {
		...
		$(".ratio").on("click", function () {
			var which = $(this).parent().attr('class').split(" ")[1], type, vol = $(this).data('volume');
			if (which == 'Sounds')
				type = efx;
			else
				type = bg;
