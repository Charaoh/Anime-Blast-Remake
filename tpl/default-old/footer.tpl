</div>

<div class="followus">
	<p>Follow us!</p>
	
	<a href="https://twitter.com/AnimeBlast3"><img src="{URL}/tpl/default/img/twitter.png"></a>
	<a href="https://www.youtube.com/channel/UCIUv_V2nKY2L45kWRcpUieQ?sub_confirmation=1"><img src="{URL}/tpl/default/img/youtube.png"/></a>
	<a href="https://discord.gg/UJCRTd2"><img src="{URL}/tpl/default/img/discord.png" /></a>
	<div class="paypal" >
		<img src="data:image/svg+xml;base64," style="
		position: absolute;
		float: left;
		top: -2px;
		width: 12px;
		padding-left: 5px;">
		<img src="data:image/svg+xml;base64," style="
		position: absolute;
		width: 59px;
		float: left;
		padding-left: 5px;
		top: -4px;">
		<span><a href="https://www.paypal.me/WShall">Help us grow!</a></span>
	</div>

<div class="third">
    <div class="ranking">
        <p class="title">Top 10 Ladder <span class="what" data-descr="This is ranked based on the players with most experience ingame. The top 3 will get a special benifit when season ends.">?</span></p>
        <ul class="list">
		{RANKED}
        </ul>
    </div>
	<div class="ranking">
        <p class="title">Top 10 Clans<span class="what" data-descr="This is ranked based on the clans with the most experience. The top 3 will get a special benifit when season ends.">?</span></p>
        <ul class="list">
		{RANKED-CLAN}
        </ul>
    </div>
	<div class="ranking">
        <p class="title">Top 10 Highest Streak<span class="what" data-descr="This is ranked based on the players with the current highest streak in the game. The top 3 will get a special benifit when season ends.">?</span></p>
        <ul class="list">
		{RANKED-STREAK}
        </ul>
    </div>
    
</div>
</div></div>
</body></html>