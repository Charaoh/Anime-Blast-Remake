        ...
        success: function(e) {
            ...
            appendStatus(a), setTimeout(function() {
                $("#selection").show("fast", function() {
                    ...
                    // EVENT 
                    if ($(".christmasImage").length)
                        $(".christmasImage").removeClass('alts');
                })
            }, 2e3)
        }
    })
        ...
        case 3:
            ...
            $(".ai").find(".difficulty").each(function() {
                $(this).on('click touchstart', function(event) {
                    ...
                    popUp("search-ai", trueDifficulty);
                })
            });
            break;
        default:
...
function popUp(e, pDifficulty = null) {
    ...
    $.ajax({
        ...
        success: function(e) {
            ...
            if ($('.passive-clones').length > 0) {
                ...
                $('.passives__moves>*').removeAttr('style');
            }
...
function getSkill(t) {
    ...
    if (t.hasClass('passive')) {
        passive = true;
        if (container.hasClass('passives__moves') && $('.passive-clones').length === 0) {
            if ($('.alts.skill-list').is(':visible')) {
                $('.skill-list').removeAttr('style');
                $('.alternatives').html("Alternative Skills");
            }
            let copyPassives = $(container).find('img').clone();
            copyPassives.removeClass('passive');
            $('.skill-list>*').not('.alts').fadeOut('fast');
            $('.skill-list').not('.alts').append("<div class='passive-clones'></div>");
            $('.passive-clones').html(copyPassives);
        }
    }
    ...
    $.ajax({
        ...
        success: function(e) {
            ...
            if (!container.hasClass('passive-clones') && !container.hasClass('passives__moves')) {
                $(".skill-list>img").removeAttr("style");
            } else {
                container.find('img').removeAttr('style');
                $(".skill-list>.passive-clones>img").removeAttr("style");
            }
            $(".character-options>img").removeAttr("style");
            $(".character-options>.passives__moves>img").removeAttr("style");
            if (passive || container.hasClass('passive-clones')) {
                $(".character-options>.passives__moves>img[id=" + t + "]").css({
                    border: "1px solid #34d0f1 "
                });
                $(".skill-list>.passive-clones>img[id=" + t + "]").css({
                    border: "1px solid #34d0f1 "
                });
            } else {
            ...
            success: function(e) {
                ...
                $(".character-page").hide("slide", {
                    ...
                    if (e.passives !== undefined) {
                        $(".character-options").append('<p class="passives">Passive Abilities</p><div class="passives__moves">' + e.passives + "</div>");
                    }
                    $(".description").html(e.description),
                        $(".character-page-bottom").html(e.stats),
                        $(".character-page").fadeIn("slow", function() { $(".character-page-bottom").fadeIn(); }),
    ...
    $.ajax({
        ...
        success: function(e) {
            ...
            if (e.error) {
                swal.stopLoading();
                swal.close();
                return errorMessage("Error: " + e.error, e.error, e.name), !1;
            }
            // Update cookies
