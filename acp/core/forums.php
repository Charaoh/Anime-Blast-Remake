...
if ($mode == 'forum-permission' && isset($_GET['id'])) {
    ...
    if (isset($_GET['edit'])) {
        ...
        $tpl = str_replace(array($STYLE->getcode('usergroups', $tpl)), '', $tpl);
    ...
    if (isset($_POST['add']) && isset($_POST['all-groups'])) {
        ...
        $db->query("INSERT INTO " . $prefix . "_forums_permission (group_id,forum_id,view,post,reply) VALUES ('" . $group . "','" . $id . "','1','1','1')");
        $system->redirect('./?s=forums&mode=forum-permission&id=' . $id . '', true);
    }
    // Remove from Allowed
    ...
    $output .= $STYLE->tags($tpl, array("L_EDIT" => L_EDIT, "ALL_GROUPS" => $all_select, "CURRENT_GROUPS" => $current_select, "L_ADD" => L_ADD, "L_REMOVE" => L_REMOVE, "L_GROUP_LIST" => L_GROUP_LIST, "L_CURRENT_GROUPS" => L_CURRENT_GROUPS, "FORUM_NAME" => $system->present($forum_data['name'])));
} else
if ($mode == 'category-permission' && isset($_GET['id'])) {
    ...
    if (isset($_POST['Submit'])) {
        ...
        if (is_null($name) || is_null($info) || is_null($category)) {
...
VALUES ('$name','$category','$info', '$parent', '$sort')");
    ...
    if (isset($_POST['Submit'])) {
        ...
        if (!$name) {
            $system->message(L_ERROR, 'Missing category name', './?s=forums&amp;mode=new-category', L_CONTINUE);
        }
