				...
				foreach ($rewards as $reward) {
					...
					foreach ($r as $m) {
						...
						if (strpos($m, 'COOKIES') !== false) {
							...
							if ($cookieTotal != '0')
								$rewarded .= '<p>You have earned ' . $cookieTotal . ' Narutomakis <img src="https://www.anime-blast.com/tpl/christmas/css/images/cookie.png" />! <br/>Congratulations for beating ' . $db->fieldFetch('accounts', $system->data('AI_Account'), 'name') . ' on ' . $difficulty . ' mode</p>';
							continue;
						}
						if (strpos($m, 'E') !== false) {
