 ...
 */
	...
	{
		global $account, $system, $user, $secure, $game, $db, $STYLE, $output, $ttime, $siteaddress, $page_title, $template, $landscape, $version;
		...
		foreach ($this->second['TEAM']['ID'] as $key => $character) {
			...
			if ($template !== 'default') {
                ...
                <!--<div><img class="shat r" src="https://cdn.discordapp.com/attachments/713810205862789405/787488168156594196/santa_hat.png" style="z-index:999999;"></div>-->
					...
					<img src="./tpl/beta/css/images/fade2.png" class="gradient r">
					{BRIEF}
				</div>
			...
			$words = array_replace($words, array_fill_keys(array_keys($words, null), ''));
			...
			if (isset($this->second["TEAM"]["STATUS"][$key]['show_mana'])) {
				...
				if ($template !== 'default') {
					...
					$briefing .= '<div class="brief mana r"><p>' . $this->second['TEAM']['MANA'][$key] . '</p></div>';
				} else
		...
		foreach ($this->first['TEAM']['ID'] as $key => $character) {
			...
			if ($template !== 'default') {
				...
				<div class="damaged">{AVATAR}</div>
					<img src="./tpl/beta/css/images/fade2.png" class="gradient flip">
					{BRIEFING}
				</div>
			...
			foreach ($this->first['TEAM']['SKILL'][$key] as $skill => $cost) {
				...
				// Cooldown
				if (isset($this->first['TEAM']['COOLDOWN'][$key][$skill]) && $this->first['TEAM']['COOLDOWN'][$key][$skill] !== '0') {
				...
				// If stunned
				if (isset($this->first['TEAM']['STATUS'][$key]['stunned']) && $this->checkStun($this->first['TEAM']['STATUS'][$key]['stunned'], $_skill['classes']) === true) {
					$opacity = 'opacity';
					goto skill;
				}
				// Disable skills
				...
				if ($_skill['requires'] !== '0') {
					...
					foreach ($active as $value) {
						...
						if (!isset($this->{$what['caster'][0]}['TEAM']['ID'][$what['caster'][1]]) || isset($this->{$what['caster'][0]}['TEAM']['ID'][$what['caster'][1]]) && $this->{$what['caster'][0]}['TEAM']['ID'][$what['caster'][1]] !== $this->first['TEAM']['ID'][$key]) continue;
				...
				$targetCondition = 'AND';
				$differentTargets = $this->returnTargets($_skill['targets']);
				if (isset($differentTargets['condition'])) {
					$targetCondition = $differentTargets['condition'];
				}
				$differentTargets = $differentTargets['targets'];
				$targets['0' . $key][$_skill['id']]['targets'] = $this->availableTargets($key, $_skill['id']);
				$targets['0' . $key][$_skill['id']]['available'] = $differentTargets;
				$targets['0' . $key][$_skill['id']]['costs'] = (($cost + $extra) < 0 ? 0 : ($cost + $extra));
				$targets['0' . $key][$_skill['id']]['condition'] = $targetCondition;
				skill:
			...
			<div class="fl-l tooltip s">
					...
					<h1>' . $_skill['name'] . '</h1>
						<p>' . $_skill['desc'] . '</p><p style="color:#32c9e9;">Mana Cost: ' . (($cost + ((isset($extra)) ? $extra : '0') < 0) ? '0' : $cost + ((isset($extra)) ? $extra : '0')) . '<br/> <span  style="color: #e63d3d;">Cooldown: ' . $_skill['cooldown'] . '</span>' . $archive . '</p>
					</div></div>' . $cooldown;
				$skillkey++;
		...
		$player = $user->image($this->first['ACCOUNT']['id'], 'avatars', './', 'fl-r avatar') . '
			...
			<p>' . $user->specialRank($this->first['ACCOUNT']['id']) . '</p>';
		$opponent = $user->image($this->second['ACCOUNT']['id'], 'avatars', './', 'fl-l avatar') . '
			...
			<p>' . $user->specialRank($this->second['ACCOUNT']['id']) . '</p>';
		if ($template !== 'default') {
	...
	function Calculate()
	{
		...
		// True Player 1 => the person attacking this turn
		$truePlayerTurn = (($this->match['TURN'] % 2) != 0 ? '1' : '0');
		// AI MATCH ->
		if ($this->match['FIRST'] === false || !$this->match['ME']) {
			$opponent = $this->first;
			$this->first = $this->second;
			$this->second = $opponent;
		}
		if ($this->match['MATCH']['type'] === 'ai' && !$this->match['ME']) {
			// Check skills of AI and process moves
			foreach ($this->first['TEAM']['ID'] as $key => $character) {
				if ($this->first['TEAM']['HEALTH'][$key] == '0') continue;
				$numSkills = 1;
				if ($this->match['MATCH']['difficulty'] == '3')
					$numSkills = 3;
				$possibleMoves = array();
				foreach ($this->first['TEAM']['SKILL'][$key] as $skill => $cost) {
					$_skill = $db->fetch('SELECT * FROM skills WHERE id = "' . $skill . '"');
					...
					if ($this->first['TEAM']['COOLDOWN'][$key][$skill] == '0' || $this->first['TEAM']['COOLDOWN'][$key][$skill] == 'None') {
						$extra = 0;
						if (isset($this->first['TEAM']['STATUS'][$key]['increase-cost']))
							$extra = $this->first['TEAM']['STATUS'][$key]['increase-cost'];
						if (($this->first['TEAM']['SKILL'][$key][$skill] + $extra) > $this->first['TEAM']['MANA'][$key])
							continue;
					}
					array_push($possibleMoves, $skill);
				}
				if (!empty($possibleMoves)) :
					while ($numSkills > 0) {
						...
						do {
							$newSelectRandIndex = array_rand($possibleMoves);
							$randIndex = $possibleMoves[$newSelectRandIndex];
							if ($this->match['MATCH']['difficulty'] > '1') {
								$this->first['TEAM']['SKILL'][$key][$randIndex] = 0;
								$this->first['TEAM']['COOLDOWN'][$key][$randIndex] = '0';
							}
							$_skill = $db->fetch('SELECT * FROM skills WHERE id = "' . $randIndex . '"');
							// If stunned
							if (isset($this->first['TEAM']['STATUS'][$key]['stunned']) && $this->checkStun($this->first['TEAM']['STATUS'][$key]['stunned'], $_skill['classes']) === true) {
								// Cover the stun so it doesn't repeat
								if (array_search($randIndex, $lastMoves) === false) {
									array_push($lastMoves, $randIndex);
									$moveCounter++;
								}
								continue;
							}
							...
							if ($this->first['TEAM']['COOLDOWN'][$key][$randIndex] == '0' || $this->first['TEAM']['COOLDOWN'][$key][$randIndex] == 'None') {
								$extra = 0;
								if (isset($this->first['TEAM']['STATUS'][$key]['increase-cost']))
									$extra = $this->first['TEAM']['STATUS'][$key]['increase-cost'];
								if (($this->first['TEAM']['SKILL'][$key][$randIndex] + $extra) > $this->first['TEAM']['MANA'][$key]) {
									if (array_search($randIndex, $lastMoves) === false) {
										array_push($lastMoves, $randIndex);
										$moveCounter++;
									}
							...
							if (array_search($randIndex, $lastMoves, TRUE) === false) {
								array_push($lastMoves, $randIndex);
								$moveCounter++;
							}
						} while ($selectedMove === false && $moveCounter !== count($possibleMoves));
						if ($selectedMove !== false) {
							$_POST['0' . $key . '-skill'] = $selectedMove;
							// Check targets and assign
							$move = $db->query("SELECT * FROM skills WHERE id = '" . $selectedMove . "'");
							if ($move->rowCount() == 0)
								continue;
							$move = $db->fetch("SELECT * FROM skills WHERE id = '" . $selectedMove . "'");
							$moveTargets = $move['targets'];
							$moveTargets = explode('|', $moveTargets);
							foreach ($moveTargets as $target) {
								if (strpos($target, 'S') !== false) {
									if (isset($_POST['0' . $key . '-targeted']))
										$_POST['0' . $key . '-targeted'] .= ',';
									else
										$_POST['0' . $key . '-targeted'] = '';
									$_POST['0' . $key . '-targeted'] .= $selectedMove;
								}
								if (strpos($target, 'A') !== false) {
									if (strpos($target, 'a') !== false) {
										$firstAlly = ($key == 0 ? 1 : ($key == 1 ? 2 : 0));
										$secondAlly = ($firstAlly == 1 ? 0 : 2);
										if (isset($_POST['0' . $firstAlly . '-targeted']))
											$_POST['0' . $firstAlly . '-targeted'] .= ',';
										else
											$_POST['0' . $firstAlly . '-targeted'] = '';
										$_POST['0' . $firstAlly . '-targeted'] .= $selectedMove;
										if (isset($_POST['0' . $secondAlly . '-targeted']))
											$_POST['0' . $secondAlly . '-targeted'] .= ',';
										else
											$_POST['0' . $secondAlly . '-targeted'] = $selectedMove;
										$_POST['0' . $secondAlly . '-targeted'] .= $selectedMove;
									} else {
										$_target = explode('/', $target);
										$_target = $_target[1];
										if ($_target == 'r') $_target = rand(0, 2);
										if (isset($_POST['0' . $_target . '-targeted']))
											$_POST['0' . $_target . '-targeted'] .= ',';
										else
											$_POST['0' . $_target . '-targeted'] = '';
										$_POST['0' . $_target . '-targeted'] .= $selectedMove;
									}
								}
								if (strpos($target, 'E') !== false) {
									if (strpos($target, 'a') !== false) {
										if (!isset($this->second['TEAM']['STATUS'][0]['invulnerability']) ||  isset($this->second['TEAM']['STATUS'][0]['invulnerability']) && $this->checkInvulnerability($this->second['TEAM']['STATUS'][0]['invulnerability'], $move['classes']) === false && $move['iinvul'] == '0') {
											if (isset($_POST['10-targeted']))
												$_POST['10-targeted'] .= ',';
											else
												$_POST['10-targeted'] = '';
											$_POST['10-targeted'] .= $selectedMove;
										}
										if (!isset($this->second['TEAM']['STATUS'][1]['invulnerability']) ||  isset($this->second['TEAM']['STATUS'][1]['invulnerability']) && $this->checkInvulnerability($this->second['TEAM']['STATUS'][1]['invulnerability'], $move['classes']) === false && $move['iinvul'] == '0') {
											if (isset($_POST['11-targeted']))
												$_POST['11-targeted'] .= ',';
											else
												$_POST['11-targeted'] = '';
											$_POST['11-targeted'] .= $selectedMove;
										}
										if (!isset($this->second['TEAM']['STATUS'][2]['invulnerability']) ||  isset($this->second['TEAM']['STATUS'][2]['invulnerability']) && $this->checkInvulnerability($this->second['TEAM']['STATUS'][2]['invulnerability'], $move['classes']) === false && $move['iinvul'] == '0') {
											if (isset($_POST['12-targeted']))
												$_POST['12-targeted'] .= ',';
											else
												$_POST['12-targeted'] = '';
											$_POST['12-targeted'] .= $selectedMove;
										}
									} else {
										$_target = explode('/', $target);
										$_target = $_target[1];
										if ($_target == 'r') $_target = rand(0, 2);
										if (!isset($this->second['TEAM']['STATUS'][$_target]['invulnerability']) ||  isset($this->second['TEAM']['STATUS'][$_target]['invulnerability']) && $this->checkInvulnerability($this->second['TEAM']['STATUS'][$_target]['invulnerability'], $move['classes']) === false && $move['iinvul'] === '0') {
											if (isset($_POST['1' . $_target . '-targeted']))
												$_POST['1' . $_target . '-targeted'] .= ',';
											else
												$_POST['1' . $_target . '-targeted'] = '';
											$_POST['1' . $_target . '-targeted'] .= $selectedMove;
										}
									}
								}
							}
		...
		foreach ($this->{$turnof}['TEAM']['ID'] as $key => $character) {
			if (isset($this->{$turnof}["TEAM"]["STATUS"][$key]['if'])) {
				...
				foreach ($all as $k => $hehe) {
					if (strpos($hehe, 'n') !== false && strpos($hehe, 'ally') == false && isset($_POST['0' . $key . '-skill']) && !is_numeric($_POST['0' . $key . '-skill'])) {
						...
						$t = (($this->match['FIRST'] == false || !$this->match['ME']) ? '1' : '0') . $key;
					...
					} elseif (strpos($hehe, 'n') == false && strpos($hehe, 'ally') == false && isset($_POST['0' . $key . '-skill']) && is_numeric($_POST['0' . $key . '-skill'])) {
		...
		if (isset($_POST['end']) && $_POST['end'] == 'true') {
			...
			$sort = false;
			$new = false;
			if (isset($_POST['order']) && $_POST['order'] !== 'X' && stripos($_POST['order'], ',') !== false) {
			...
			foreach ($sort as $key) {
				...
				// Skill was used on this character check if copy is active
				// Copy skill?
				if (isset($this->first["TEAM"]["STATUS"][$key]['copy']) && $this->first["TEAM"]["STATUS"][$key]['copy'][2] == "harmful") {
					// Check to see if its not the same skill
					// Check classes of skill, if all copy if not check specifics
					$classes = explode(',', $skill['classes']);
					$specific = explode(',', $this->first["TEAM"]["STATUS"][$key]['copy'][0]);
					$found = false;
					foreach ($specific as $s) {
						if ($s == 'all')
							$found = true;
					...
					if ($found === true) {
						// If has classes then process the copy effect
						$this->first["TEAM"]["STATUS"][$key]['copy'][3] = $skill['id'];
					}
				}
				/*if(isset($this->first['TEAM']['STATUS'][$key]['targetme']))
					$skill['targets'] = $this->first['TEAM']['STATUS'][$key]['targetme'][2];*/
				// Before going into the targets... check if the character used a skill that activates on others
				$skillTargets = explode("|", $skill['targets']);
				$firstTarget = null;
				$secondTarget = null;
				$mainTarget = null;
				foreach ($skillTargets as $t) {
					if ($t === "targetOthers") {
						// Find the first targeted by the skill and target others
						foreach ($targets as $id => $target) {
							...
							foreach ($targeted as $me) {
								if ($me == 'X')
									continue;
								if ($me !== $skill['id'])
									continue;
								// The skills main target found
								$mainTarget = $__;
							}
						}
						if ($mainTarget !== null) {
							$firstTarget = ($mainTarget === "0" ? "1" : ($mainTarget === "1" ? "2" : "0"));
							$secondTarget = $firstTarget + 1;
							if ($secondTarget > 2)
								$secondTarget = "0";
							$firstTarget = "1" . $firstTarget;
							$secondTarget = "1" . $secondTarget;
							$targets[$firstTarget] = ($targets[$firstTarget] === "X" ? $skill['id'] : $targets[$firstTarget] . "," . $skill['id']);
							$targets[$secondTarget] = ($targets[$secondTarget] === "X" ? $skill['id'] : $targets[$secondTarget] . "," . $skill['id']);
						}
					}
				}
				foreach ($targets as $id =>  &$target) {
					...
					foreach ($targeted as $w =>  &$me) {
						...
						if ($random === true) {
							...
							do {
								$__ = rand(0, 2);
								if (array_search($__, $allTargets) === false) {
									array_push($allTargets, $__);
								} else
									continue;
								if ($this->{$identifier}['TEAM']['HEALTH'][$__] !== '0')
									if (isset($this->{$identifier}['TEAM']['STATUS'][$__]['invulnerability']) && $this->checkInvulnerability($this->{$identifier}['TEAM']['STATUS'][$__]['invulnerability'], $skill['classes']) === false)
										$blank = true;
								$totalTrys++;
							} while ($blank === false && $totalTrys === 3);
							$l = $skill['id'];
						}
						//Countered
						$counter = false;
						...
						if ($skill['uncounterable'] == '1')
							$counter = true;
						if (isset($this->{$identifier}['TEAM']['STATUS'][$__]['counter']) && strpos($this->{$identifier}['TEAM']['STATUS'][$__]['counter'], 's') !== false)
							$counter = true;
						if (isset($this->first['TEAM']['ID'][$key]['no-ignore']) && strpos($this->first['TEAM']['ID'][$key]['no-ignore'], 'counter') !== false)
							$counter = false;
						if (isset($this->{$identifier}['TEAM']['STATUS'][$__]['counter']) && $this->{$identifier}['ACCOUNT']['id'] !== $this->first['ACCOUNT']['id'] && $counter === false) {
							...
							foreach ($checkme as $k => $v) {
								...
								if (isset($this->{$identifier}['TEAM']['STATUS'][$__]['counter']) && $this->{$identifier}['TEAM']['STATUS'][$__]['counter'] != key($t['skill']))
									continue;
								foreach (reset($t['skill']) as $effect => $turnsleft) {
									...
									if (!empty($e['counter'])) {
										...
										if ($e['counter'] !== 'Nothing') {
											...
											$me = $truePlayerTurn;
											$castedBy = ($me == '0' ? '1' : '0');
											$return = $this->match['TURN'] . '=' . $me . $__ . ':' . $e['counter'] . '[' . $w . ']' .  $castedBy . $key;
										}
							...
							foreach ($checkme as $k => $v) {
								...
								foreach (reset($t['skill']) as $effect => $turnsleft) {
									...
									if (!empty($e['counter'])) {
										...
										$me = ($truePlayerTurn == '1' ? '0' : '1');
										$return = $this->match['TURN'] . '=' . $truePlayerTurn . $__ . ':' . $e['counter'] . '[' . $w . ']' . $me . $key;
						...
						foreach ($effects as $effect) {
							...
							if (strpos($data['target'], '>') !== false) {
								$specialValue = substr($data['target'], strpos($data['target'], '>') + 1);
								$data['target'] = substr($data['target'], 0, strpos($data['target'], '>'));
								$condition = ">";
							}
							if (strpos($data['target'], '<') !== false) {
								$specialValue = substr($data['target'], strpos($data['target'], '<') + 1);
							...
							switch ($data['target']) {
								case 'S':
									if ($this->{$identifier}['TEAM']['ID'][$__] !== $this->first['TEAM']['ID'][$key] || $this->{$identifier}['ACCOUNT']['id'] !== $this->first['ACCOUNT']['id'])
										$continue = true;
									$self = true;
									break;
								case 'A':
									if (!in_array($this->{$identifier}['TEAM']['ID'][$__], $this->first['TEAM']['ID']) || $identifier == 'second' || $this->{$identifier}['TEAM']['ID'][$__] == $this->first['TEAM']['ID'][$key])
										$continue = true;
									break;
								case 'E':
									if (!in_array($this->{$identifier}['TEAM']['ID'][$__], $this->second['TEAM']['ID']) || $identifier == 'first')
										$continue = true;
									if ($mainTarget !== null) {
										if ($firstTarget === $_ . $__)
											$continue = true;
										if ($secondTarget === $_ . $__)
											$continue = true;
									}
									break;
								case 'O':
								...
								case 'M':
									...
									if ($condition === "")
										$continue = true;
									if ($condition == ">") {
										if ($this->{$identifier}['TEAM']['MANA'][$__] < $specialValue) {
											$continue = true;
										}
									}
									if ($condition == "<") {
						...
						if (!empty($_add)) {
							$ignore = false;
							if (isset($this->{$identifier}['TEAM']['STATUS'][$__]['ignore']) && strpos($this->{$identifier}['TEAM']['STATUS'][$__]['ignore'], 'reflect') !== false)
								$ignore = true;
							if ($skill['unreflectable'] == '1')
								$ignore = true;
							if (isset($this->{$identifier}['TEAM']['STATUS'][$__]['reflect'][1]) && $this->{$identifier}['TEAM']['STATUS'][$__]['reflect'][1] !== 'all') {
								$classes = explode(',', $skill['classes']);
								$specifics = explode(',', $this->{$identifier}['TEAM']['STATUS'][$__]['reflect'][1]);
								$found = false;
							...
							if (isset($this->{$identifier}['TEAM']['STATUS'][$__]['no-ignore']) && strpos($this->{$identifier}['TEAM']['STATUS'][$__]['no-ignore'], 'reflect') !== false)
								$ignore = false;
							// Reflect back
							if (isset($this->{$identifier}['TEAM']['STATUS'][$__]['reflect']) && $this->{$identifier}['ACCOUNT']['id'] != $this->first['ACCOUNT']['id'] && $ignore === false) {
								$checkme = $active;
								foreach ($checkme as $k => $v) {
									$t = $this->makemeunderstand($v);
									if ($this->{$identifier}['TEAM']['STATUS'][$__]['reflect'][0] != key($t['skill'])) continue;
									foreach (reset($t['skill']) as $effect => $turnsleft) {
										$e = $db->fetch("SELECT * FROM effects WHERE id = '" . $effect . "'");
										if ($e['count'] !== '1')
											unset($this->{$identifier}['TEAM']['STATUS'][$__]['reflect']);
									}
									$t['skill'][key($t['skill'])][$effect] = 'e';
									$t = $this->makemeunderstand($t);
									$checkme[$k] = $t;
								...
								$active = $checkme;
								$finalTarget = ($truePlayerTurn == '0' ? '1' : '0');
								$active[] = $this->match['TURN'] . '=' . $truePlayerTurn . $__ . ':' . $skill['id'] . '[' . $_add . ']' . $finalTarget . $key;

								if ($this->{$identifier}['TEAM']['STATUS'][$__]['reflect'][2] !== 'all') {
									$this->{$identifier}['TEAM']['STATUS'][$__]['reflect'][2] = $this->{$identifier}['TEAM']['STATUS'][$__]['reflect'][2] - 1;
									if ($this->{$identifier}['TEAM']['STATUS'][$__]['reflect'][2] <= 0)
										unset($this->{$identifier}['TEAM']['STATUS'][$__]['reflect']);
								}
							} else {
								// Skill was used on this character check if copy is active
								// Copy skill?
								if (isset($this->{$identifier}["TEAM"]["STATUS"][$__]['copy']) && $this->{$identifier}["TEAM"]["STATUS"][$__]['copy'][2] == "friendly") {
									if ($this->first["ACCOUNT"]["id"] !== $this->{$identifier}["ACCOUNT"]["id"]) {
										// Check to see if its not the same skill
										// Check classes of skill, if all copy if not check specifics
										$classes = explode(',', $skill['classes']);
										$specific = explode(',', $this->{$identifier}["TEAM"]["STATUS"][$__]['copy'][0]);
										$found = false;
										foreach ($specific as $s) {
											if ($s == 'all')
												$found = true;
										...
										if ($found === true) {
											// If has classes then process the copy effect
											$this->{$identifier}["TEAM"]["STATUS"][$__]['copy'][3] = $skill['id'];
										}
									}
								}
								$active[] = $this->match['TURN'] . '=' . (($this->match['FIRST'] == true || !$this->match['ME']) ? 0 : 1) . $key . ':' . $skill['id'] . '[' . $_add . ']' . (($this->match['FIRST'] == false || !$this->match['ME']) ? (($_ == 0) ? 1 : 0) : $_) . $__;
							}
						}
					}
				}
		...
		foreach ($active as $key => &$value) {
			...
			foreach (reset($what['skill']) as $effect => $turnsleft) {
				...
				if (!is_numeric($turnsleft) && $turnsleft !== 'infinite') {
					...
					if (
						isset($this->{$what['caster'][0]}['TEAM']['STATUS'][$what['caster'][1]]['stunned']) &&
						$this->checkStun(
							$this->{$what['caster'][0]}['TEAM']['STATUS'][$what['caster'][1]]['stunned'],
							$db->fieldFetch('skills', key($what['skill']), 'classes')
						) !== false && strpos($turnsleft, 's') !== false
					) {
					...
					if (
						isset($this->{$what['caster'][0]}['TEAM']['STATUS'][$what['caster'][1]]['invulnerability']) &&
						$this->checkInvulnerability(
							$this->{$what['caster'][0]}['TEAM']['STATUS'][$what['caster'][1]]['invulnerability'],
							$db->fieldFetch('skills', key($what['skill']), 'classes')
						) !== false && strpos($turnsleft, 'i') !== false
					) {
					...
					foreach ($backed as $k => $v) {
						...
						if ($what['target'][0] == $t['target'][0] || $what['target'][0] == $t['caster'][0]) {
							...
							if ($s === 'target') {
								...
								if (array_keys($t['skill'])[0] == $trueReturn) {
									// check if looking for effect specific

									$removed[] = $k;
								} else {

									if (strpos($trueReturn, '+') !== false) {
										$returned = substr($trueReturn, strpos($trueReturn, '+') + 1);
										$check = false;
										foreach (reset($t['skill']) as $ef => $tl) {
											$cResult = $db->fieldFetch('effects', $ef, $returned);
											if ($cResult !== 'undefined' && !empty($cResult))
												$check = true;
										}
										if ($check === true)
											$removed[] = $k;
		...
		foreach ($active as $key => &$value) {
			...
			foreach (reset($what['skill']) as $effect => $turnsleft) {
				...
				$checkCopy = $db->fieldFetch('effects', $effect, 'copy');
				if ($checkCopy !== "undefined" && $checkCopy == "1" && $_value == "")
					$_value = 1;
				if (isset($_value) && $_value !== '') {
					...
					if (isset($this->{$what['target'][0]}['TEAM']['STATUS'][$what['target'][1]]['copy'])) {
						// Check to see if its the same active to update the effect value
						$copyKeyArray = explode(',', $this->{$what['target'][0]}['TEAM']['STATUS'][$what['target'][1]]['copy'][1]);
						$copyKey = array_search($key, $copyKeyArray);
						if ($copyKey === false) continue;
						if (isset($this->{$what['target'][0]}["TEAM"]["STATUS"][$what['target'][1]]['copy'][3]))
							$_value = $this->{$what['target'][0]}["TEAM"]["STATUS"][$what['target'][1]]['copy'][3];
						else
							continue;
					}
	...
	function returnTargets($pSkillTargets)
	{
		$return = array();
		$targets = explode('|', $pSkillTargets);
		...
		$return['targets'] = implode('|', $targets);
	...
	function availableTargets($character, $skill)
	{
		...
		$cost = $cost + $extra;
		$differentTargets = $this->returnTargets($skill['targets']);
		$differentTargets = $differentTargets['targets'];
		$targets = explode('|', $differentTargets);
			...
			switch ($target) {
				...
				case "12";
					...
					if ($this->{(($m == '1') ? 'second' : 'first')}['TEAM']['HEALTH'][$k] == 0 && isset($this->{(($m == '1') ? 'second' : 'first')}['TEAM']['STATUS'][$k]['no-resurrect']))
						break;
					if (isset($this->{(($m == '1') ? 'second' : 'first')}['TEAM']['STATUS'][$k]['invulnerability']) && $this->checkInvulnerability($this->{(($m == '1') ? 'second' : 'first')}['TEAM']['STATUS'][$k]['invulnerability'], $skill['classes']) !== false && $skill['iinvul'] !== '1')
						break;
					$targeting[$m . $k] = true;
					break;
				case "E/1";
				...
				case "E/a";
					foreach ($this->second['TEAM']['ID'] as $key => $person) {
						...
						if ($this->second['TEAM']['HEALTH'][$key] == 0 && isset($this->second['TEAM']['STATUS'][$key]['no-resurrect']))
							continue;
						if (
							isset($this->second['TEAM']['STATUS'][$key]['invulnerability']) &&
							$this->checkInvulnerability($this->second['TEAM']['STATUS'][$key]['invulnerability'], $skill['classes']) !== false && $skill['iinvul'] !== '1'
						)
							continue;
						if (isset($this->first["TEAM"]["STATUS"][$character]['targetme']) && strpos($this->first["TEAM"]["STATUS"][$character]['targetme'][2], 'not') !== false) {
...
class GAjax
{
	function getUI()
	{
		...
		foreach ($this->second['TEAM']['ID'] as $key => $character) {
			...
			if (isset($this->second["TEAM"]["STATUS"][$key]['show_mana'])) {
				...
				if ($left > 100)
					$left = 100;
				$team['1' . $key]['bar'] = '<div class="brief mana r"><p>' . $this->second['TEAM']['MANA'][$key] . '</p></div>';
			}
		...
		foreach ($this->first['TEAM']['ID'] as $key => $character) {
			...
			$healths['0' . $key]['background'] = $background;
			if ($this->first['TEAM']['HEALTH'][$key] > '0') {
				$healths['0' . $key]['bar']['brief'] = '<div class="brief health"' . $background . '><p>' . $this->first['TEAM']['HEALTH'][$key] . '</p></div>';
			}
			$left = round(($this->first['TEAM']['MANA'][$key] / $db->fieldFetch('characters', $character, 'mana')) * 100);
			if ($left > 100)
				$left = 100;
			...
			$mana['0' . $key]['width'] = $left;
			if ($this->first['TEAM']['HEALTH'][$key] > '0') {
				$mana['0' . $key]['bar']['brief'] = '<div class="brief mana"><p>' . $this->first['TEAM']['MANA'][$key] . '</p></div>';
				$mana['0' . $key]['bar']['full'] = '<div id="' . $left . '" class="manaBar">
				<p class="error ' . $character . '">For the first turn there is a mana cap of ' . $system->data('First_Mana') . ' per character. <br/> For turn 2 there is a mana cap of ' . $system->data('Second_Mana') . ' per character. </p>
				   <div class="manaBar Left" style="width: ' . $left . '%"><p>' . $this->first['TEAM']['MANA'][$key] . '</p></div></div>';
			}
			...
			foreach ($this->first['TEAM']['SKILL'][$key] as $skill => $cost) {
				...
				if ($_skill['requires'] !== '0') {
					...
					foreach ($active as $value) {
						...
						if (isset($this->{$what['caster'][0]}['TEAM']['ID'][$what['caster'][1]]) && $this->{$what['caster'][0]}['TEAM']['ID'][$what['caster'][1]] !== $this->first['TEAM']['ID'][$key]) continue;
				...
				$team['targets']['0' . $key][$_skill['id']]['available'] = $differentTargets;
				$team['targets']['0' . $key][$_skill['id']]['costs'] = (($cost + $extra) < 0 ? 0 : ($cost + $extra));
				$team['targets']['0' . $key][$_skill['id']]['condition'] = $targetCondition;
				skill:
	...
	function verifySkill()
	{
		...
		if ($s['requires'] != '0') {
			...
			foreach ($active as $value) {
				$what = ($this->match['FIRST'] == true)  ? $this->makemeunderstand($value) : $this->makemeunderstand($value, 1);
				if ($this->{$what['caster'][0]}['ACCOUNT']['id'] !== $this->first['ACCOUNT']['id']) continue;
				if (!isset($this->{$what['caster'][0]}['TEAM']['ID'][$what['caster'][1]]) || isset($this->{$what['caster'][0]}['TEAM']['ID'][$what['caster'][1]]) && $this->{$what['caster'][0]}['TEAM']['ID'][$what['caster'][1]] !== $this->first['TEAM']['ID'][$what['caster'][1]]) continue;
				if (($llave = array_search(key($what['skill']), $requires)) !== false) {
					unset($requires[$llave]);
				}
			}
		...
		if (!isset($this->first['TEAM']['COOLDOWN'][$character][$_POST['s']])) $this->first['TEAM']['COOLDOWN'][$character][$_POST['s']] = "0";
		...
		if (isset($this->first['TEAM']['STATUS'][$character]['stunned']) && $this->checkStun($this->first['TEAM']['STATUS'][$character]['stunned'], $s['classes']) !== false)
			return $json['result'] = false;
		// Disable skills
		$skillkey = 1;
		foreach ($this->first['TEAM']['SKILL'][$character] as $skill => $cost) {
			if ($skill == $s['id'])
				break;
			$skillkey++;
		}
			...
			switch ($target) {
				...
				case "E/a";
					foreach ($this->second['TEAM']['ID'] as $key => $person) {
						...
						if ($this->second['TEAM']['HEALTH'][$key] == 0 && isset($this->second['TEAM']['STATUS'][$key]['no-resurrect']))
							continue;
						if (isset($this->second['TEAM']['STATUS'][$key]['invulnerability']) && $this->checkInvulnerability($this->second['TEAM']['STATUS'][$key]['invulnerability'], $skill['classes']) !== false && $skill['iinvul'] !== '1')
							continue;
						if (isset($this->first["TEAM"]["STATUS"][$character]['targetme']) && strpos($this->first["TEAM"]["STATUS"][$character]['targetme'][2], 'not') !== false) {
							if (strpos($this->first["TEAM"]["STATUS"][$character]['targetme'][3], 'all') === false) {
								$specifics = explode(',', $this->first["TEAM"]["STATUS"][$character]['targetme'][3]);
		...
		foreach ($targets as $target) {
			switch ($target) {
				...
				case "12";
					...
					break;
				case "E/1":
					if (isset($this->second['TEAM']['STATUS'][$posible[$defined]]['invulnerability']) && $this->checkInvulnerability($this->second['TEAM']['STATUS'][$posible[$defined]]['invulnerability'], $skill['classes']) !== false && $skill['iinvul'] !== '1')
						break;
...
class Functions extends GAjax
{
		...
		foreach ($data as $effect => $value) {
			...
			switch ($effect) {
				...
				case 'deal':
					...
					break;
				case 'no-death':
					if (isset($this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['reverseTargetToCaster'])) {
				...
				case 'disable':
					...
					$this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['disable'] = $this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['disable'] . $disable;
					$success = true;
					break;
				...
				case 'manaGain':
					if (
						isset($this->{$arguments[1][0]}["TEAM"]["STATUS"][$arguments[1][1]]['stunned']) &&
						$this->checkStun($this->{$arguments[1][0]}["TEAM"]["STATUS"][$arguments[1][1]]['stunned'], $db->fieldFetch('skills', $arguments[4], 'classes')) !== false
					) {
						$success = true;
						break;
					}
				...
				case 'drainM':
					if (
						isset($this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['invulnerability']) &&
						$this->checkInvulnerability($this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['invulnerability'], $db->fieldFetch('skills', $arguments[4], 'classes')) !== false && $db->fieldFetch('skills', $arguments[4], 'iinvul') !== '1'
					) {
						$success = true;
						break;
					}
					if (isset($this->{$arguments[1][0]}["TEAM"]["STATUS"][$arguments[1][1]]['stunned']) && $this->checkStun($this->{$arguments[1][0]}["TEAM"]["STATUS"][$arguments[1][1]]['stunned'], $db->fieldFetch('skills', $arguments[4], 'classes')) !== false) {
						$success = true;
						break;
					}
					$manaGain = $value;
				...
				case 'drainH':
					if (isset($this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['invulnerability']) && $this->checkInvulnerability($this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['invulnerability'], $db->fieldFetch('skills', $arguments[4], 'classes')) !== false && $db->fieldFetch('skills', $arguments[4], 'iinvul') !== '1') {
						$success = true;
						break;
					}
					if (isset($this->{$arguments[1][0]}["TEAM"]["STATUS"][$arguments[1][1]]['stunned']) && $this->checkStun($this->{$arguments[1][0]}["TEAM"]["STATUS"][$arguments[1][1]]['stunned'], $db->fieldFetch('skills', $arguments[4], 'classes')) !== false) {
				...
				case 'convertM':
					if (
						isset($this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['invulnerability'])
						&& $this->checkInvulnerability($this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['invulnerability'], $db->fieldFetch('skills', $arguments[4], 'classes')) !== false
						&& $db->fieldFetch('skills', $arguments[4], 'iinvul') !== '1'
					) {
						$success = true;
						break;
					}
				...
				case 'heal':
					...
					if ($this->{$arguments[0][0]}["TEAM"]["HEALTH"][$arguments[0][1]] <= 0 && empty($db->fieldFetch('skills', $arguments[4], 'dead'))) {
						...
						if ($this->{$arguments[0][0]}["TEAM"]["HEALTH"][$arguments[0][1]] === 0 && $heal > 0)
							$this->{$arguments[1][0]}["TEAM"]["STATUS"]['resurrected'] = true;
				...
				case 'reflect':
					...
					if ($arguments[1][0] == $arguments[0][0]) {
						...
						$this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['reflect'][2] = (empty($data['count']) ? 1 : $data['count']);
					}
					$success = true;
					break;
				...
				case 'replace':
					...
					break;
				case 'copy':
					$identifier = (empty($data['self']) ? $arguments[0][0] : $arguments[1][0]);
					$identifierKey = (empty($data['self']) ? $arguments[0][1] : $arguments[1][1]);
					if ($arguments[3] === '0') {
						if (!empty($_value)) {
							$pos = array_search($_value, array_keys($this->{$identifier}["TEAM"]["SKILL"][$identifierKey]), true);
							$skills = explode(',', $db->fieldFetch('characters', $this->{$identifier}["TEAM"]["ID"][$identifierKey], 'skills'));
							$new = array($skills[$data['key']] => $db->fieldFetch('skills', $skills[$data['key']], 'cost'));
							$this->{$identifier}["TEAM"]["SKILL"][$identifierKey] = array_slice($this->{$identifier}["TEAM"]["SKILL"][$identifierKey], 0, $pos, TRUE) + $new + array_slice($this->{$identifier}["TEAM"]["SKILL"][$identifierKey], $pos + 1, count($this->{$identifier}["TEAM"]["SKILL"][$identifierKey]), TRUE);
							unset($this->{$identifier}["TEAM"]["SKILL"][$identifierKey][$pos]);
						}
					} else {
						if (empty($_value)) {
							if (isset($this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['copy'][0])) {
								$this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['copy'][0] .= ',';
								$this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['copy'][1] .= ',';
								$this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['copy'][2] .= ',';
							}
							$this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['copy'][0] .= (empty($data['specify']) ? 'all' : $data['specify']);
							$this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['copy'][1] .= $arguments[5];
							$this->{$arguments[0][0]}["TEAM"]["STATUS"][$arguments[0][1]]['copy'][2] .= ($this->{$arguments[0][0]}['ACCOUNT']['id'] == $this->{$arguments[1][0]}['ACCOUNT']['id']) ? "friendly" : "harmful";
						} else {
							if (array_search($_value, array_keys($this->{$identifier}["TEAM"]["SKILL"][$identifierKey]), true) === false) {
								$pos = $data['key'];
								$new = array($_value => $db->fieldFetch('skills', $_value, 'cost'));
								$this->{$identifier}["TEAM"]["SKILL"][$identifierKey] = array_slice($this->{$identifier}["TEAM"]["SKILL"][$identifierKey], 0, $pos, TRUE) + $new + array_slice($this->{$identifier}["TEAM"]["SKILL"][$identifierKey], $pos + 1, count($this->{$identifier}["TEAM"]["SKILL"][$identifierKey]), TRUE);
								unset($this->{$identifier}["TEAM"]["SKILL"][$identifierKey][$pos]);
							}
						}
	...
	function ReturnDescription($arguments, $directory)
	{
		...
		$target = "Undefined";
		if (!isset($this->{$arguments['target'][0]}['TEAM']['ID'][$arguments['target'][1]])) return false;
		$target = $db->query("SELECT * FROM characters WHERE id = '" . $this->{$arguments['target'][0]}['TEAM']['ID'][$arguments['target'][1]] . "'");
		if ($target->rowCount() === 0)
			return false;
		$target = $target->fetch();
		$caster = "Undefined";
		if (!isset($this->{$arguments['caster'][0]}['TEAM']['ID'][$arguments['caster'][1]])) return false;
			...
			foreach ($data as $id => $it) {
				...
				switch ($id) {
					...
					case 'deal':
						...
						if ($it == 'all')
							$pre .= '<p>- <b>' . $target['name'] . '</b> will take ' . $data['increaseby'] . ' more damage.' . $this->defineTurns($_turns, $turns) . '</p>';
						else
						...
						if (!empty($data['specify'])) {
							...
							foreach ($specific as $k => $we) {
								...
								if ($k == 0)
									...
									$specific .= (($k + 1 == $counter) ? ' or ' : ', ') . $db->fieldFetch('classes', $we, 'name');
							}
							$specific .= " ";
						}
						$who = ($data['self'] == "1" ? $caster['name'] : $target['name']);
						if ($custom != "")
							$pre .= '<p>- <b>' . $who . '</b> has copied <b>' . $db->fieldFetch("skills", $custom, "name") . '</b>. ' . $this->defineTurns($_turns, $turns) . '</p>';
						else {
							if ($arguments['target'][0] !== $arguments['caster'][0])
								$pre .= '<p>- <b>' . $who . '</b> will copy any ' . $specific . 'skill used by this character. ' . $this->defineTurns($_turns, $turns) . '</p>';
							else
								$pre .= '<p>- <b>' . $who . '</b> will copy any ' . $specific . 'skill used on this character. ' . $this->defineTurns($_turns, $turns) . '</p>';
						}
						break;
					case 'dr':
						if (is_numeric($tturns) && round($tturns - (($this->match['TURN'] - $_turns) / 2)) == 0) {
					...
					case 'heal':
						if (!empty($data['condition'])) {
							if ($data['condition'] == 'H0') {
								$pre .= '<p>- <b>' . $target['name'] . '</b> will heal ' . (($data['heal'] == 'f') ? 'to full health' : $data['heal']) . ' when reaching 0 health. ' . $this->defineTurns($_turns, $turns) . '</p>';
								break;
							}
						}
					...
					case 'reflect':
						if ($turns === 'e') {
							$skill['invisible'] = '0';
							$pre .= '<p>- This skill has been reflected successfully</p>';
						} else {
							if (round($tturns - (($this->match['TURN'] - $_turns) / 2)) == 0) {
								$pre .= '<p>- This skill has ended</p>';
								$skill['invisible'] = '0';
								...
								$pre .= '<p>- <b>' . $target['name'] . '</b> will reflect ' . ((empty($data['count']) || !empty($data['count']) && $data['count'] == 1) ? 'the first skill with ' . $specific . ' skill classes' : 'all skills with ' . $specific . ' skill classes') . $this->defineTurns($_turns, $turns) . '</p>';
							}
						}
						break;
					case 'ignore':
			...
			if (!is_numeric($lasts)) {
				...
				switch ($lasts) {
					...
					case 'm':
						...
						break;
					case 'infinite':
						$return .= 'this skill will not end';
						break;
					default:
						$return .= 'Undefined';
						break;
